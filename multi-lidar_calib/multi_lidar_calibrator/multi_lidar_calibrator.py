...
class MultiLidarCalibrator(Node):
    ...
    def __init__(self):
        ...
        self.lidar_data = {}    # lidars msgs
        self.lidar_dict = {}    # lidar pcd 
        self.subscribers = []
        self.counter = 0
        ...
        if [len(self.lidar_data[i]) == self.frame_count for i in self.lidar_data.keys()] == [
            ...
            if self.counter >= self.runs_count:
                if self.to_stitch_cloud:
                    from multi_lidar_calibrator.stitch_cloud.sync_stitch_clouds import SyncAndStitchCloudsPub
                    stitcher = SyncAndStitchCloudsPub(
                        topic_names=self.topic_names,
                        target_lidar=self.target_lidar,
                        tf_Result=self.tf_Result
                    )
                    print(f"stitch clouds: {self.topic_names} at {self.target_lidar},with tf: {self.tf_Result}")
                    rclpy.spin(stitcher)
